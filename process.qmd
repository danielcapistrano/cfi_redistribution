...
ess_ie <- 
    ess_ie |> 
        mutate(
            ess_month = case_when(     
                essround < 3 ~ as.Date(paste0(inwyr, "-", inwmm, "-01"), format = "%Y-%M-%d"),
                essround > 2 & essround  < 10 ~ as.Date(paste0(inwyys, "-", inwmms, "-01"), format = "%Y-%M-%d"),
                essround > 9 ~ as.Date(format(inwds, "%Y-%M-%d"), format = "%Y-%M-%d")
            )
        ) |>
